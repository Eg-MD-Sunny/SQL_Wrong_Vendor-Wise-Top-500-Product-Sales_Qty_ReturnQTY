select top 500 pv.id [PVID],
		pv.Name [Product],
		pvp.CostPrice [CostPrice],
		pvp.CachedListPrice [ListPrice],
		Concat(Convert(decimal(10,2),((pvp.CachedListPrice-pvp.CostPrice))/pvp.CachedListPrice),'%')[Margin],
		v.id [VID],
		v.Name [VendorName]
...
and s.ReconciledOn < '2022-08-04 00:00 +06:00'
...
group by pv.id,
		 ...
		 pvp.CostPrice,
		 pvp.CachedListPrice,
		 Concat(Convert(decimal(10,2),((pvp.CachedListPrice-pvp.CostPrice))/pvp.CachedListPrice),'%'),
		 v.id,
		 v.Name 
